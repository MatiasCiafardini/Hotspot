                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.removed_ingredients.length > 0 && (
                            <p className="text-xs text-muted-foreground truncate">Sin: {item.removed_ingredients.join(", ")}</p>
                          )}
...
function CustomizeDialog({
  ...
  const toggleIngredient = (ingredient: string, checked: boolean) => {
    ...
    setRemoved((current) =>
      checked ? [...current, ingredient] : current.filter((currentIngredient) => currentIngredient !== ingredient),
    );
  };
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[90vh] overflow-y-auto border-ink bg-background">
        <DialogHeader>
          <DialogTitle className="font-display text-3xl uppercase">Personalizar {item.name}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            <p className="mb-2 text-sm font-bold uppercase text-muted-foreground">Quitar ingredientes</p>
            <div className="grid gap-2 sm:grid-cols-2">
              {item.base_ingredients.map((ingredient) => {
                ...
                return (
                  <label
                    key={ingredient}
                    className={`flex items-center gap-2 rounded-md border border-ink/30 bg-card p-3 text-sm ${
                  ...
                  >
                    ...
                    {locked && <span className="text-[10px] uppercase text-muted-foreground">Fijo</span>}
                  </label>
                );
              })}
            </div>
          ...
          />
          <SmashButton onClick={() => onSave({ removed_ingredients: removed.filter(canRemoveIngredient), item_notes: notes })} className="w-full">
