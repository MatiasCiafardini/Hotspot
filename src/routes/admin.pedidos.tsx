...
export const Route = createFileRoute("/admin/pedidos")({
  head: () => ({ meta: [{ title: "Pedidos admin - Hotspot" }, { name: "robots", content: "noindex" }] }),
  component: OrdersPage,
});

const ACTIVE_ORDER_STATUSES: OrderStatus[] = ["pending", "pending_payment", "pending_confirmation", "confirmed", "preparing", "ready"];
const ACTIONABLE_ORDER_STATUSES: OrderStatus[] = ["pending", "pending_payment", "pending_confirmation"];
...
function OrdersPage() {
  ...
  const load = useCallback(async (options?: { notifyNew?: boolean; silentErrors?: boolean }) => {
    ...
    if (options?.notifyNew && hasLoadedRef.current) {
      const newOrders = nextOrders.filter((order) => !previousIds.has(order.id) && ACTIONABLE_ORDER_STATUSES.includes(order.status));
      if (newOrders.length > 0) {
        toast.info(newOrders.length === 1 ? "Nuevo pedido recibido." : `${newOrders.length} pedidos nuevos recibidos.`);
      }
    }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "orders" }, () => load({ notifyNew: true }))
  ...
  const updateStatus = async (order: AdminOrder, status: OrderStatus) => {
    ...
    const patch = status === "confirmed" ? { status, payment_status: "approved" } : { status };
    const { error } = await (supabase as any).from("orders").update(patch).eq("id", order.id);
    if (error) {
      whatsappWindow?.close();
      return toast.error("No se pudo actualizar el pedido.");
    }
    toast.success(status === "confirmed" ? "Pedido confirmado. Comanda lista." : `Pedido: ${ORDER_STATUS_LABEL[status]}`);
    const updatedOrder = { ...order, ...patch };
    setOrders((current) => getActiveOrders(current.map((item) => (item.id === order.id ? { ...item, ...patch } : item))));
  ...
  return (
    <>
      ...
      <div className="mb-4 rounded-lg border border-orange-400/30 bg-orange-500/10 p-4 text-sm text-orange-100">
        {pending.length} pedidos necesitan revision. Al confirmar, el estado cambia y podes descargar o imprimir la comanda.
      </div>

      <div className="grid gap-4">
        {orders.map((order) => (
          <article key={order.id} className="rounded-lg border border-white/10 bg-zinc-900/80 p-4 shadow-lg">
            {(() => {
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`rounded-full border px-2 py-1 text-xs ${ORDER_STATUS_CLASS[order.status]}`}>
                ...
                <p className="mt-1 text-sm text-zinc-400">
                  ...
                  {order.customer_address ? ` · ${order.customer_address}` : " · Retiro en local"}
                </p>
                <p className="mt-1 text-sm text-zinc-400">
                  Pago {order.payment_method || "A confirmar"} · {PAYMENT_STATUS_LABEL[order.payment_status || "pending"]}
                  {order.payment_receipt_url && (
                    <a className="ml-2 text-orange-300 underline" href={order.payment_receipt_url} target="_blank" rel="noreferrer">
                      Ver comprobante
                    </a>
                  )}
                </p>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {canConfirmOrReject && (
                  <>
                    ...
                    <AdminButton variant="danger" onClick={() => updateStatus(order, "rejected")}>
                      <X className="h-4 w-4" /> Rechazar
                    </AdminButton>
                  </>
                )}
                <AdminButton variant="ghost" onClick={() => downloadComandaPdf(order, settings)}>
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_200px]">
                ...
                {order.order_items?.map((item) => (
                  <div key={item.id} className="rounded-md border border-white/10 bg-black/30 p-3">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <p className="font-mono text-sm text-zinc-400">{formatMoney(item.unit_price * item.quantity)}</p>
                    </div>
                    <div className="mt-2 grid gap-1 text-xs text-zinc-400 md:grid-cols-2">
                      <p>Base: {item.base_ingredients?.length ? item.base_ingredients.join(", ") : "Sin detalle"}</p>
                      <p>Quitados: {item.removed_ingredients?.length ? item.removed_ingredients.join(", ") : "Ninguno"}</p>
                      <p>Agregados: {item.added_ingredients?.length ? item.added_ingredients.join(", ") : "Ninguno"}</p>
              ...
              <div className="rounded-md border border-white/10 bg-black/30 p-3">
                ...
                <div className="mt-3">
                  <AdminSelect value={order.status} onChange={(event) => updateStatus(order, event.target.value as OrderStatus)}>
        ...
        {!loading && orders.length === 0 && (
          <div className="rounded-lg border border-white/10 bg-zinc-900/70 p-10 text-center text-zinc-400">No hay pedidos pendientes.</div>
        )}
      </div>
        ...
        <div className="mt-5 space-y-3">
          {order.order_items?.map((item) => (
            <div key={item.id} className="rounded-md border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-2 text-sm text-zinc-400">Ingredientes base: {item.base_ingredients?.join(", ") || "Sin detalle"}</p>
              <p className="text-sm text-zinc-400">Quitados: {item.removed_ingredients?.join(", ") || "Ninguno"}</p>
              <p className="text-sm text-zinc-400">Agregados: {item.added_ingredients?.join(", ") || "Ninguno"}</p>
              <p className="text-sm text-zinc-400">Observaciones: {item.item_notes || "Sin observaciones"}</p>
            </div>
          ))}
        </div>
